"""
最终版本 - 直接解析m3u8（不通过浏览器模拟）
完整流程：获取ConFig → 解密URL → 跟踪重定向 → 获取最终m3u8
"""
...
class FinalDirectParser:
    ...
    def decrypt_url(self, encrypted_url: str, uid: str) -> Optional[str]:
        """
        解密ConFig.url
        使用AES-CBC解密，匹配CryptoJS/NotGm的行为
        
        CryptoJS/NotGm行为：
        1. key = CryptoJS.enc.Utf8.parse('2890' + uid + 'tB959C')
        2. iv = CryptoJS.enc.Utf8.parse("2F131BE91247866E")
        3. CryptoJS.AES.decrypt()会自动处理密钥长度，如果不是16/24/32字节，会使用MD5哈希
        
        注意：如果Python解密失败，建议使用 browser_decrypt_parser.py 在浏览器中执行JavaScript解密
        """
        print(f"\n[步骤3] 解密ConFig.url...")
        ...
        iv_methods = [
            ...
            ("十六进制解析(8字节+填充)", bytes.fromhex(iv_str).ljust(16, b'\0')),
        ]
        ...
        print(f"\n   ❌ 所有组合都失败了")
        print(f"\n   💡 建议使用浏览器自动化方案:")
        print(f"      python browser_decrypt_parser.py")
        print(f"   或者查看更详细的解密尝试:")
        print(f"      python final_direct_parser_v2.py")
        return None
    ...
    def get_final_m3u8(self, api_url: str) -> Optional[str]:
        ...
        try:
            ...
            if content.strip().startswith('#EXTM3U'):
                ...
                # 保存m3u8内容
                with open('final_m3u8_final.m3u8', 'w', encoding='utf-8') as f:
                    f.write(content)
                print(f"   💾 已保存到: final_m3u8_final.m3u8")
                
                return final_url
            
            # 如果URL包含m3u8关键字，即使内容不是标准格式也返回
            if '.m3u8' in final_url or 'm3u8' in final_url.lower():
                print(f"   ⚠️ API响应不是标准m3u8格式")
                print(f"   💡 但URL包含m3u8关键字，返回此URL")
                print(f"   📄 响应预览: {content[:200]}")
                return final_url
            
            print(f"   ⚠️ API响应不是m3u8格式")
            print(f"   📄 响应预览: {content[:200]}")
            return None
    ...
    def parse_video(self, parser_url: str, video_url: str) -> Optional[str]:
        ...
        try:
            ...
            # 保存HTML
            with open('iframe_page_final.html', 'w', encoding='utf-8') as f:
                f.write(html)
            ...
            if final_m3u8:
                ...
                with open('final_parse_result.json', 'w', encoding='utf-8') as f:
                    json.dump(result, f, indent=2, ensure_ascii=False)
                print(f"\n✅ 完整结果已保存到: final_parse_result.json")
